"""Tests for cycle timing accounting (Section 10)."""
...
def test_under_budget_not_flagged():
    t = build_cycle_timing(0.5, 8.0, 0.2, dt_s=0.05)  # 8.7 < 50ms
    assert t.over_budget is False


def test_over_budget_flagged():
    t = build_cycle_timing(10.0, 50.0, 5.0, dt_s=0.05)  # 65 > 50ms
...
def test_explicit_total_used():
    ...
    # accounted is the segment sum, distinct from measured total
    assert abs(t.accounted_ms - 8.7) < 1e-9
