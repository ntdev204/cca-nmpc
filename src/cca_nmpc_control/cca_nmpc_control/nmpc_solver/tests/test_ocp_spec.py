"""Tests for the backend-agnostic OCP spec (SV-02)."""
import math
...
def test_dynamics_match_eq_4_1_at_sample():
    # theta=0: xdot=vx, ydot=vy, thetadot=omega
    spec = ocp_spec.OCPSpec(dt=0.05)
    x = np.array([0.0, 0.0, 0.0])
    u = np.array([1.0, 0.5, 0.2])
    x_next = spec.integrate_numpy(x, u)
    # small-step Euler-ish check: position advances in +x,+y, heading in +theta
    assert x_next[0] > 0 and x_next[1] > 0 and x_next[2] > 0
    # at theta=0 forward velocity maps directly to x
...
def test_dynamics_rotation_at_theta_90():
    ...
    u = np.array([1.0, 0.0, 0.0])  # forward at heading 90deg -> +y
    x_next = spec.integrate_numpy(x, u)
    assert x_next[1] > 0
